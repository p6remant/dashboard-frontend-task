  ...
  return (
    ...
    >
              ...
              {filteredUsers.map((user) => (
                <li key={user.id}>
                  <Link
                    ...
                    className={searchResultRowClassName}
      ...
      {activeCategory !== SearchCategory.Users && productsSectionVisible && (
        <SearchResultSection
          title="In Products"
          onClear={onClearProductsSection}
        >
      ...
      {/**
        OTHER (commented out — matches UI: “OTHER” + “Search for … in Users/Products” rows).
       Restore with `isOtherVisible` / `onClearOtherSection` from Header and `QuickSearchLink` import.
       
       {isOtherVisible && (
        <SearchResultSection title="Other" onClear={onClearOtherSection}>
           <ul className="space-y-0.5 px-1 pb-2">
            <QuickSearchLink query={query} scope="Users" to={...} onSelectResult={onSelectResult} />
            <QuickSearchLink query={query} scope="Products" to={...} onSelectResult={onSelectResult} />
            </ul>
          </SearchResultSection>
        )}
       */}
